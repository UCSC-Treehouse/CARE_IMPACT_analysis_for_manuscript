...
```{r}
# 
# disease_group_for_plot <- sample_metadata %>%
#   select(donorID, dx_group) %>% 
#   left_join(disease_group_names, by=c("dx_group"="original_disease_group_name")) %>%
#   group_by(dx_group) %>%
#   mutate(dx_group_num = n()) %>%
#   ungroup %>%
#   arrange(order) %>%
#   mutate(long_disease_group_name_for_plot_n = 
#            paste(long_disease_group_name_for_plot_n, " (n=", dx_group_num, ")"),
#          long_disease_group_name_for_plot_n = 
#            factor(long_disease_group_name_for_plot_n, levels = unique(long_disease_group_name_for_plot_n) ))
...
ggplot(finding_categories_for_plot) +
  ...
  facet_row(~molecular_category_type,
            scales = "free_x",
            space = "free",
            labeller = labeller(molecular_category_type=intial_cap_no_underscore) )  + 
  theme_minimal() +
