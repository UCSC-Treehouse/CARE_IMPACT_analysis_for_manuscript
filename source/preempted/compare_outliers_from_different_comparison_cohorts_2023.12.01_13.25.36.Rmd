...
n_distinct(sample_metadata$THid)
n_distinct(sample_metadata$donorID)
...
outlier_upset <- outliers %>%
  mutate(index = paste(Sample_ID, gene),
         value = TRUE) %>%
  select(index, value,comparison_cohort) %>%
  pivot_wider(names_from = comparison_cohort) %>%
  select(index, TCGA, Pediatric = PEDAYA, Stanford = TH03_TH34, Treehouse_pc, Treehouse_pd) %>%
  replace_na(list(TCGA = FALSE, Pediatric = FALSE, Stanford = FALSE, Treehouse_pc = FALSE,
                  Treehouse_pd = FALSE))
...
outlier_upset %>%
  ...
  upset(text.scale=1.5, 
        mainbar.y.label = "Outliers detected by\ncohort combination",
...
# While X outliers were detected by multiple cohorts...
```{r}

cohorts_per_outlier <- outliers %>%
  mutate(index = paste(Sample_ID, gene)) %>%
  group_by(index) %>%
  summarize(n_cohorts = n()) 


tabyl(cohorts_per_outlier$n_cohorts)
