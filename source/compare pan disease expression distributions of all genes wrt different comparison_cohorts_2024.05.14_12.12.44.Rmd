...
patient_expression_from_compendia <- bind_rows(outliers_to_keep$gene,
...
# Show VEGFA expression distributions in TCGA PEDAYA and Treehouse_pc cohorts for TH34_1445_S02
```{r}
outliers_to_plot_set <-outliers_to_plot %>%
  filter(gene == "VEGFA",
         Sample_ID == "TH34_1445_S02")

t4 <- pmap(list(outliers_to_plot_set$gene, outliers_to_plot_set$Sample_ID, outliers_to_plot_set$outlier_relative_to), plot_distributions)


this_sample_id <- "TH34_1445_S02"
this_gene <- "VEGFA"
 
relevant_cohorts <- c("TCGA", "PEDAYA", "Treehouse_pc")
possible_levels <- relevant_cohorts %>%
  update_cohort_strings

expr_relevant_to_this_outlier_raw <-  left_join(
  samples_in_cohorts %>%
    filter(cohort %in% relevant_cohorts),
  expr %>%
    filter(Gene == this_gene),
  by=c("Sample_ID")) %>%
  mutate(cohort = update_cohort_strings(cohort))

table(expr_relevant_to_this_outlier_raw$cohort)
...
plot_output_name <- paste0("../results/Expression of TH34_1445_S02 VEGFA in 3 cohorts ", format(Sys.time(), "%Y_%m_%d-%H_%M_%S_%Y"), ".png")
